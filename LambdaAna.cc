...
void LambdaAna::event(BelleEvent* evptr, int* status) {
  ...
  */
    ...
    for(Mdst_vee2_Manager::iterator vee_it=Mdst_vee2_Mgr.begin();vee_it!=Mdst_vee2_Mgr.end();vee_it++)
      {
    findLtmp.candidates((*vee_it),ip_position);
    ...
    allParticlesNonBoosted.push_back(ch0_fit.vect());
    allParticlesNonBoosted.push_back(ch1_fit.vect());
    ch0_fit.boost(kinematics::CMBoost);
    ch1_fit.boost(kinematics::CMBoost);
    allParticlesBoosted.push_back(ch0_fit.vect());
    allParticlesBoosted.push_back(ch1_fit.vect());
    trkIndex_used.push_back((*vee_it).chgd(0).get_ID());
    trkIndex_used.push_back((*vee_it).chgd(1).get_ID());
    visEnergy_lab+=ch0_fit.e();
    visEnergy_lab+=ch1_fit.e();
    }
    if(nLcan<1) return; //no Lambda candidate
    ...
    int ii=0;
     ...
     for(Mdst_charged_Manager::iterator chr_it=Mdst_charged_Mgr.begin();chr_it!=Mdst_charged_Mgr.end();chr_it++,ii++)
      {
       bool isUsed=false;
       for(int iUsed=0;iUsed<trkIndex_used.size();iUsed++)
       {
      ...
      if(isUsed) { /*cout<<"alreday in the particle pool as Lambda daughters "<<endl;*/continue; }
       ...
       if(mu_id>mu_cut&& e_id>e_cut)
        ...
        getDrDz(chr_it, massHyp,dr,dz, refitPx, refitPy, refitPz);
         if ( fabs(dr) > cuts::vertexR  || fabs(dz) > cuts::vertexZ )//slides from kibayashi 
          {
            continue;
          }
  ...
  int ngood = allParticlesBoosted.size();
   ...
   for(int igam =0; igam<gamma_mgr.size();igam++)
	...
	Mdst_ecl_aux &aux =eclaux_mgr(Panther_ID(gam.ecl().get_ID()));
       ...
       if(gammaE<cuts::minGammaEBarrel) continue;

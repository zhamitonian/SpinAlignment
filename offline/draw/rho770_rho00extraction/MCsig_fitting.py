...
Double_t rho_omega_signal(Double_t m, Double_t A, Double_t B, Double_t C, Double_t phase) {{
    ...
    double Gr = {Gamma_rho_0} * ({m_rho}/m) * std::pow((m2 - {4*m_pi**2}) / ({m_rho**2} - {4*m_pi**2}), 1.5);
    double Gw = {Gamma_omega_0} * ({m_omega}/m) * std::pow((m2 - {4*m_pi**2}) / ({m_omega**2} - {4*m_pi**2}), 1.5);

    // Breit-Wigner amplitudes: sqrt(m*m0*Gamma) / (m0^2 -m^2 - i*m0*Gamma)
    cd bw_rho = std::sqrt(m * {m_rho} * Gr) / cd({m_rho**2} - m2, -{m_rho} * Gr);
    cd bw_omega = std::sqrt(m * {m_omega} * Gw) / cd({m_omega**2} - m2, -{m_omega} * Gw);
...
""")
    ...
    pdf_config = FitDefinition([Variable("rho_M", 0.770 - 0.150, 0.770 + 0.150, 120)],
                [PDFSpec("sig", "rho_M", "generic_pdf",
                        config = {"formula": f"rho_omega_signal(rho_M, A[1,0,10], B[0.1,-5,5], C[1,0,10], phase[0,0,{2*pi}])"}),],
                model = "sig")

    plot_config = PlotConfiguration(plot_config={
        "xlabel" : {"rho_M" : "M_{#pi^{+}#pi^{-}} (GeV/c^{2})"},
        "components" : {"sig" : {"label" : "Total Fit", "color" : 4},},

...
def wrapper_func(tree, output_dir, log_file, bin_fit_range, branches_name, binned_fit):
    ...
    pdf_config = FitDefinition([Variable("Ks_M", 0.47, 0.52, 200)],
                ...
                PDFSpec("gauss", "Ks_M", "gaussian",
                        {"mean":"mean", "sigma":"sigma * k[1, 0.001, 100]"}),
                #PDFSpec("reso_gauss", "Ks_M", "bifur_gauss", {"mean":(0, -0.1, 0.1), "sigma_left":(0.01, 0.0001, 0.1), "sigma_right":(0.01, 0.0001, 0.1)}),
                PDFSpec("reso_gauss", "Ks_M", "double_gauss", {"mean1":(0, -0.1, 0.1), "sigma1":(0.01, 0.0001, 0.1), "sigma2":(0.01, 0.0001, 0.1),"frac":(0.5,0, 1), "same_mean":True}),
                PDFSpec("bkg", "Ks_M", "chebychev", {"order":1, "coef1": (0, -bkg_range, bkg_range)}),],
                model = f"SUM(nsig[{n_entry}, {0.5 * n_entry}, {1.5 * n_entry}]*FCONV(Ks_M, SUM(frac[0.3,0,0.8]*DSCB, gauss), reso_gauss), nbkg[{0.05 * n_entry},0, {0.1 * n_entry}]*bkg)")

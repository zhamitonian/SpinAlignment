...
def wrapper_func(tree, output_dir, log_file, bin_fit_range, branches_name, binned_fit):
    ...
    hist = R.TH1F("", ";X;#chi^{2}/ndof", 10, 0, 10) 
    pad_width = len(str(hist.GetNbinsX()))
    for i in range(hist.GetNbinsX()):
        alpha_devi = i * 0.5 + 1

        if alpha_devi + n_devi == 0:
            pdf_config = FitDefinition([Variable("Ks_M", 0.47, 0.52, 200)],
                        [PDFSpec("DSCB", "Ks_M", "crystal_ball",
                                    {"mean":("mean[0.497,0.496,0.498] + diff[0, -0.01, 0.01]"),
                        ...
                        PDFSpec("gauss", "Ks_M", "gaussian",
                                {"mean":"mean + diff", "sigma":"sigma * A * k[1, 0.001, 100]"}),
            ...
            pdf_config = FitDefinition([Variable("Ks_M", 0.47, 0.52, 200)],
                        [PDFSpec("DSCB", "Ks_M", "crystal_ball",
                                    {"mean":("mean[0.497,0.496,0.498] + diff[0, -0.01, 0.01]"),
                                    "sigma":("sigma[0.006,0.001,0.03] * A[1,0.7,1.3] *(1-a[0, -1, 1])"),
                                    "alpha":(f"alpha_l[1.5,0.01,5] * alpha_l_div[1, {1/alpha_devi}, {alpha_devi}]"),
                                    "n":(f"nr[2.0,0.01,100] * nr_div[1, {1/n_devi}, {n_devi}]"),
                                    "n_right":(f"nl[2.0,0.01,100] * nl_div[1, {1/n_devi}, {n_devi}]"),
                                    "alpha_right":(f"alpha_r[1.5,0.01,5] * alpha_r_div[1, {1/alpha_devi}, {alpha_devi}]"),
                                    "sigma_right":"sigma*A*(1+a)"}),
                        PDFSpec("gauss", "Ks_M", "gaussian",
                                {"mean":"mean + diff", "sigma":"sigma * A * k[1, 0.001, 100]"}),
                        PDFSpec("bkg", "Ks_M", "chebychev", {"order":1, "coef1": (0, -bkg_range, bkg_range)}),],
        ...
        fitter_data = GenericFit(tree, output_dir + f"_{i:0{pad_width}d}", log_file=log_file_i, fit_definition=pdf_config,

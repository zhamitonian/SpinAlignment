...
def truth_level_rho00_vs_p(truth_rootFile,output_dir):
    os.makedirs(output_dir, exist_ok=True)
    ...
    hist_model = R.RDF.TH1DModel("", ";cos#theta*;N_{sig}", 10, -1, 1)
    for i in range(12):
        hist = rdf.Define("temp",f"Ks_helicity_angle[Ks_p >= {0.18 + i*0.1} && Ks_p < {0.28 + (i+1)*0.1}]").Histo1D(hist_model , "temp").GetValue()
        value, err = fit_rho00(hist, os.path.join(output_dir, f"fit_truth_{i:02d}.png"), True, extra_legend=f"{0.18 + i*0.1:.2f} < p < {0.28 + (i+1)*0.1:.2f}")
        p_values.append(0.05 + i*0.1 + 0.18)
        rho00_values.append(value)
        rho00_errors.append(err)
    
    hist_rho00_p = R.TH1F("hist_rho00_p", ";p;#rho_{00}", 12, 0.18, 1.38)
    for i in range(len(p_values)):
        bin_x = hist_rho00_p.GetXaxis().FindBin(p_values[i])
        hist_rho00_p.SetBinContent(bin_x, rho00_values[i])
        hist_rho00_p.SetBinError(bin_x, rho00_errors[i])
    
    c = style_draw([hist_rho00_p], "", styles=[HistStyle.error_bars(R.kBlack)], y_min = 0, y_max=0.6, use_user_y_range=True, save = False)
    line = R.TLine(0.18, 1/3, 1.38, 1/3)
    line.SetLineColor(R.kRed)
    line.SetLineStyle(R.kDashed)
    line.SetLineWidth(2)
    line.Draw("same")

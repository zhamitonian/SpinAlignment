...
def calculate_eff_original(truth_rootFile, nsig_txt, output_dir):
    ...
    hist_model = R.RDF.TH2DModel("", ";cos#theta*;z", 10, -1, 1, 7, 0.3, 0.65)
    ...
    hist_nsig = R.TH2D("hist_nsig", "nsig in cos#theta vs Ks z;cos#theta*;Ks z", 10, -1, 1, 7, 0.3, 0.65)
...
def extract_rho00(nsig_txt:str, output_dir:str, eff_root:str = None):
    ...
    df_valid = df_valid[(df_valid['z_center'] >= 0.3) & (df_valid['z_center'] <= 0.65)].reset_index(drop=True)
...
if __name__ == "__main__":
    truth_rootFile = "/gpfs/group/belle/users/wangz/data_gMC_belle1/KsSpinAlignment_v2.3.2_qqbar/exp55_truth_processed.root"
    nsig_txt = "data_v2.3.2/output/nsig_results.txt"

    nsig_mc_txt = "qqbar_v2.3.2/output/nsig_results.txt"
    nsig_mc_txt_withoud_correction = "qqbar_without_PID_eff_correction_v2.3.2/output/nsig_results.txt"

    eff_dir = "qqbar_v2.3.2/eff"
    eff_without_correction_dir = "qqbar_without_PID_eff_correction_v2.3.2/eff"
    os.makedirs(eff_dir, exist_ok=True)
    os.makedirs(eff_without_correction_dir, exist_ok=True)
    
    if not os.path.exists(os.path.join(eff_dir, "efficiency_2D.root")):
        calculate_eff_original(truth_rootFile, nsig_mc_txt, eff_dir)
        calculate_eff_original(truth_rootFile, nsig_mc_txt_withoud_correction, eff_without_correction_dir)

    rho00_output_dir = "data_v2.3.2/rho00"
    rho00_output_dir_without_correction = "data_v2.3.2/rho00_without_PID_eff_correction"
    os.makedirs(rho00_output_dir, exist_ok=True)
    os.makedirs(rho00_output_dir_without_correction, exist_ok=True)
    
    h1 = extract_rho00(nsig_txt, rho00_output_dir, os.path.join(eff_dir, "efficiency_2D.root"))
    """
    h2 = extract_rho00(nsig_txt, rho00_output_dir_without_correction, os.path.join(eff_without_correction_dir, "efficiency_2D.root"))
    c = style_draw([h1, h2], "", styles=[HistStyle.error_bars(R.kBlue), HistStyle.error_bars(R.kRed)], leg_texts=["With correction", "Without correction"], y_min=0, y_max=0.6, use_user_y_range=True, save=False)

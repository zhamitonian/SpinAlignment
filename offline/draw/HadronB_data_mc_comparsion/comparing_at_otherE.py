...
class comparing:
    ...
    """
    def __init__(self, output_dir, data_dir, qqbar_dir):
    ...
    def brush(self, hist_list, output_dir, var, leg_position=2, normalize=False):
        ...
        c_all.Update()
        style_draw([ h_ccbar, h_uds, h_data], "", 
                   ['c#bar{c}' , 'uds','data'],
                   styles = [HistStyle.filled_hist(3, 3011),
    ...
    def start_plotting(self):
        ...
        #for var in ["h_Esum"]:
            ...
            # Check if histograms were successfully loaded
            if not data_hist:
                print(f"Error: Failed to load data histogram for {var}, skipping...")
                continue
            if not uds_hist:
                print(f"Error: Failed to load UDS histogram for {var}, skipping...")
                continue
            if not ccbar_hist:
                print(f"Error: Failed to load charm histogram for {var}, skipping...")
                continue
            
            uds_hist.Scale(1.0/4)
            ccbar_hist.Scale(1.0/4)

            hist_list = [data_hist, uds_hist, ccbar_hist]
...
if __name__ == "__main__":
    ...
    config = {
        "1S_offres" : [ f"{qqbar_dir}/1S_offres", f"{data_dir}/1S_offres"],
        "2S_offres" : [ f"{qqbar_dir}/2S_offres", f"{data_dir}/2S_offres"],
        "3S_offres" : [ f"{qqbar_dir}/3S_offres", f"{data_dir}/3S_offres"],
    }
    for key in config:
        comparing_instance = comparing(
            output_dir = f"./images_v1.0.2/{key}",

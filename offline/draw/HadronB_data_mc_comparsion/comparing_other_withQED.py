...
class comparing:
    ...
    """
    def __init__(self, output_dir, data_dir, qqbar_dir, qed_dir, luminosity):
        ...
        self.luminosity_4S = 89482.240 # in pb^-1
        pass
    ...
    def _get_qed_hist(self, var): 
        ...
        combined_tautau_hist.Scale(self.luminosity / self.luminosity_4S)
        combined_other_qed_hist.Scale(self.luminosity / self.luminosity_4S)
        print(f"Scaled qed mc histograms by factor {self.luminosity / self.luminosity_4S} to match luminosity {self.luminosity} pb^-1")
...
if __name__ == "__main__":
    qqbar_dir = "/gpfs/group/belle/users/wangz/data_gMC_belle1/HadronB_DataMC_comp_v1.0.2_qqbar"
    data_dir = "/gpfs/group/belle/users/wangz/data_gMC_belle1/HadronB_DataMC_comp_v1.0.2_data"
    qed_dir = "/gpfs/group/belle/users/wangz/data_gMC_belle1/HadronB_DataMC_comp_v1.0.2_qed/rootFiles"

    config = {
        "1S_offres" : [ f"{qqbar_dir}/1S_offres", f"{data_dir}/1S_offres", 1815.620],
        "2S_offres" : [ f"{qqbar_dir}/2S_offres", f"{data_dir}/2S_offres", 1708.410],
        "3S_offres" : [ f"{qqbar_dir}/3S_offres", f"{data_dir}/3S_offres", 246.060],
    }
    for key in config:
        comparing_instance = comparing(
            output_dir = f"./images_v1.0.2_withQED/{key}",
            data_dir = config[key][1],
            qqbar_dir= config[key][0],
            qed_dir = qed_dir,
            luminosity = config[key][2])

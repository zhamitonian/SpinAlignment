...
class comapring_by_exp:
    ...
    def start_plotting(self):
        ...
        for var in ['nGood', 'nECL', 'nPhoton', 'Evis_cms', 'Esum_cms', 'PzSum_cms', 'Evis', 'Esum', 'PzSum',
                ...
                'cls_p', 'cls_theta', 'cls_phi', 'pho_p', 'pho_theta', 'pho_phi', 'trk_dr', 'trk_dz', 'PrimeVr']:
            ...
            rootFile.cd()
            h_data.Write(f"data_{var}")
            h_uds.Write(f"uds_{var}")
            h_ccbar.Write(f"ccbar_{var}")
            if self.correct_dz:
                h_sideband.Write(f"data_sideband_{var}")
            rootFile.Close()

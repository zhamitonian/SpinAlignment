...
/**
 * Constructor for the LambdaAna class.
 * Initializes member variables, output file paths, and vectors used for event weighting.
 * Sets up random number generators with fixed seeds for reproducibility.
 */
LambdaAna::LambdaAna() {
    puts( "***** [LambdaAna] Module loaded successfully." );
    //strcpy(rFileName,"notInitialized.root");
    m_debug=false;  // Debug mode flag
    m_mc=0;         // 0 for data, 1 for MC sample
    m_rm_mctree=1;  // Flag to control MC truth tree creation (1=remove)
    m_rm_tree=0;    // Flag to control analysis tree creation (0=keep)
    m_rm_mixtree=0; // Flag to control mixed event tree creation (0=keep)
    m_PID = true;   // Enable particle identification
    //m_mc=new char[20];
    m_output_filename=new char[256];        // Allocate space for output filename
    m_output_filename_genTree=new char[256]; // Allocate space for generator tree filename
    
    // Clear vectors used for polarization weighting and event selection
 ...
 * This method sets default values for beam energies, kinematic cuts, and physics parameters.
 * @param int* Unused parameter (required by the framework)
 */
void LambdaAna::init(int *) {
    printf( "\n[LambdaAna] parameters\n" );
    eler=0;               // Low Energy Ring beam energy (will be set later)
    eher=0;               // High Energy Ring beam energy (will be set later)
    //m_rm_mctree=true;
    m_thrustMagCut=0.8;   // Cut on thrust magnitude
    m_zhcut=0.2;          // Lower bound for momentum fraction z
    m_zhcut_up=0.9;       // Upper bound for momentum fraction z
    m_inputPolarization=-0.05;  // Input Lambda polarization for weighting
    m_LamDecayPar = 0.642;      // Lambda decay parameter
    m_antiLamDecayPar = -0.71;  // Anti-Lambda decay parameter
    m_PI = 3.1415926;     // Definition of π
 ...
 * Initializes beam energies and calculates center-of-mass kinematics.
 * @param evptr Pointer to the Belle event
 * @param status Status indicator
 */
void LambdaAna::begin_run(BelleEvent* evptr, int *status) {
    eid::init_data();          // Initialize electron identification
    (void)evptr; (void)status; // Avoid unused parameter warnings

    IpProfile::begin_run();    // Initialize interaction point profile
    BeamEnergy::begin_run();   // Initialize beam energy information

    eler=BeamEnergy::E_LER();  // Get Low Energy Ring beam energy
    eher=BeamEnergy::E_HER();  // Get High Energy Ring beam energy
   
    // Check if beam energies are within expected range
    ...
    // Calculate center-of-mass kinematics with crossing angle
    //double theta(0.022);
    double theta=BeamEnergy::Cross_angle();  // Get beam crossing angle
  
    // Create center-of-mass Lorentz vector
    kinematics::cm=HepLorentzVector(-eher*sin(theta),0.,-eher*cos(theta)+eler,eher+eler);
    kinematics::CMBoost=kinematics::cm.boostVector();  // Calculate boost vector to CM frame
  
    // Create Lorentz vectors for electron beams
...
void LambdaAna::hist_def() {
  ...
  #define ADDMCBRANCH(x,type)            ADDBRANCH__(x,x,x/type)
  #define ADDMCBARRAY(x,n,type)          ADDBARRAY__(x,x,n,type)
 ...
 * This is where the core event processing happens - particle selection, 
 * Lambda reconstruction, and physics observables calculation.
 * 
 * The event analysis workflow:
 * 1. Retrieve basic event information (experiment, run, event numbers)
 * 2. Clear containers from previous events
 * 3. Check interaction point profile is usable
 * 4. Find Lambda candidates in the event
 * 5. Identify other particles (pions, kaons) for correlation studies
 * 6. Calculate thrust axis for event orientation
 * 7. For each Lambda candidate:
 *    - Calculate kinematic variables
 *    - Compute angles in various reference frames
 *    - Perform polarization analysis
 *    - Match to MC truth information (if MC mode)
 *    - Store results in output trees
 * 8. Perform mixed-event analysis for background studies
 * 
 * @param evptr Pointer to the Belle event
 * @param status Status indicator
 */
void LambdaAna::event(BelleEvent* evptr, int* status) {
    ...
    // Retrieve experiment, event, and run numbers
    ...
    // Clear particle containers for each event
    ppion.clear(); // Vector for pion particles
    pkaon.clear(); // Vector for kaon particles
    // Print event information every 10000 events for monitoring
    if(!(countEvt%10000)) 
        cout << "evt " <<countEvt<< " expNo "<< expNo << ", runNo "<< runNo << ", evtNo "<< evtNo <<endl;
    
    // Clear vectors used for event weighting (for polarization analysis)
    weightedProton.clear(); weightedPion.clear();
    weightedProton_zptcom.clear(); weightedPion_zptcom.clear(); 
    weightedProton_zptcom_b.clear(); weightedPion_zptcom_b.clear(); 
    weightedProton_zptcom_c.clear(); weightedPion_zptcom_c.clear();
    weightedFactor_lambdaId.clear(); weightedFactor_factor.clear();
   
    // Clear vectors for hadron-Lambda correlation studies
    comweightedProton_zptcom_href.clear(); comweightedPion_zptcom_href.clear(); 
    comweighted_Hadron_zptcom_href.clear();
    comweightedProton_zptcom_href_b.clear(); comweightedPion_zptcom_href_b.clear(); 
    comweighted_Hadron_zptcom_href_b.clear();
   
   // Initialize MC truth variables
   Hep3Vector truthThrustAxisCMS(0,0,0);
   Hep3Vector qqAxisCMS(0,0,0); 
   int numquark;
   
   // If MC mode enabled, read MC truth information
   if(m_mc==1) { 
     // Read MC truth before applying detector efficiency
     truthThrustAxisCMS = readMC(qqAxisCMS, numquark);
   }
   
   // Check consistency of weighted particle containers
   if(comweightedProton_zptcom_href.size() != comweighted_Hadron_zptcom_href.size()) 
     cout<<"somethig is wrong, size of dropped pair is inconsistent "<<endl;
  ...
  */
    ...
    // Check if interaction point profile is usable, exit if not
    if(!IpProfile::usable()) {cout <<" ip not usable ..." << endl;return;}
    
    // Get interaction point position and error matrix
    const HepPoint3D ip_position = IpProfile::e_position();
    const HepSymMatrix ip_osition_err = IpProfile::e_position_err();
    countEvt ++;

    // Prepare vectors to store all particle momenta (boosted to CMS and non-boosted)
    ...
    // Get managers for charged tracks, V0 candidates, and track information
    ...
    FindLambda findLtmp; int nLcan=0;  // temp usage
...
for(Mdst_vee2_Manager::iterator vee_it=Mdst_vee2_Mgr.begin(); vee_it!=Mdst_vee2_Mgr.end(); vee_it++) {
        // --------------------------- The Evis part of track energy -------------------------------
        // --------------------------- E of charged track of good lambda's two daughters -------
    findLtmp.candidates((*vee_it), ip_position);  // best candidate ?, find lambda0 candidates ? 
    int goodlamda = findLtmp.goodLambda();
    //if(goodlamda!=1&&goodlamda!=2) continue;
    if(goodlamda!=2) continue;   // w means lambda kind ? 2 for lambda ,3 for anti-lambda ?
    nLcan++; // lambda candidates
    ...
    allParticlesNonBoosted.push_back(ch0_fit.vect());  // .vect() gives Hep3Vector
    allParticlesNonBoosted.push_back(ch1_fit.vect());
    ch0_fit.boost(kinematics::CMBoost);  // boost to CMS frame 
    ch1_fit.boost(kinematics::CMBoost);
    allParticlesBoosted.push_back(ch0_fit.vect());
    allParticlesBoosted.push_back(ch1_fit.vect());
    trkIndex_used.push_back((*vee_it).chgd(0).get_ID()); // .chgd(),  access to the charged track
    trkIndex_used.push_back((*vee_it).chgd(1).get_ID()); // .getid() ,  get identifier of the track 
    visEnergy_lab += ch0_fit.e(); // .e() , even after boost, still get the lab E ?
    visEnergy_lab += ch1_fit.e(); // could different lambda candidate have same daughter patially ?
}
if(nLcan<1) return; 
    atc_pid selKPi(3,1,5,3,2);  //K/pi separation
    atc_pid selPiP(3,1,5,2,4); 
    atc_pid selKP(3,1,5,3,4); // 3 1 5 means what ?
    ...
    for(Mdst_charged_Manager::iterator chr_it=Mdst_charged_Mgr.begin();chr_it!=Mdst_charged_Mgr.end();chr_it++,ii++) {
        bool isUsed=false;
        for(int iUsed=0;iUsed<trkIndex_used.size();iUsed++) { // why not use a set directly
            if((*chr_it).get_ID()==trkIndex_used[iUsed]) {  isUsed=true; break;}
        }
        if(isUsed) { /*cout<<"alreday in the particle pool as Lambda daughters "<<endl;*/continue; }
        double m_mass=m_pi;
        int massHyp=2;  // mass hypothesis e mu pi k p 
        bool isLepton=false;
        ...
        if(e_id>e_cut && mu_id<mu_cut) {
            if(m_debug) cout <<"is electron" <<endl; 
            m_mass=m_e;
            massHyp=0;
        ...
        if (fabs(dr) > cuts::vertexR  || fabs(dz) > cuts::vertexZ) {  // dr dz cut
            continue;
        }
        ...
        double E=sqrt(m_mass*m_mass+h3Vect.mag2());  // make sense ?
        ...
        if(positivelyIdentified&&massHyp==3) {
            ikaon.push_back(ii); vkaonprob.push_back(atcKP); vp4kaon.push_back(boostedVec);vp4kaon_lab.push_back(LabVec); kaonchr.push_back(charge); kaonTrkID.push_back(TrkID); pkaon.push_back(p);
        }
        allParticlesBoosted.push_back(boostedVec.vect());
        allParticlesNonBoosted.push_back(h3Vect); // other charged track
        nonBoostedE.push_back(E);
        visEnergy_lab+=E; }
    //end of loop  charged particles

   
  int ngood = allParticlesBoosted.size(); // good lambda's daughter and other good track count
  // ntrack


// ----------------------- photon , ecl ---------------------------------------------------------  
    Mdst_gamma_Manager& gamma_mgr=Mdst_gamma_Manager::get_manager();
    Mdst_ecl_aux_Manager& eclaux_mgr = Mdst_ecl_aux_Manager::get_manager(); // ecl auxiliary data
   ...
   for(int igam =0; igam<gamma_mgr.size();igam++)
{
    ...
    int id=(int)gam.get_ID(); // get identifier of specific gamma
    double px=gam.px();
    double py=gam.py();
    double pz=gam.pz();
    Mdst_ecl_aux &aux =eclaux_mgr(Panther_ID(gam.ecl().get_ID()));  // coordinate ecl of the detected "gamma"
    if(gam.ecl().quality()!=0) continue;  // remove bad quality ecl ? or removed matched ecl ? 
    double e9oe25 =aux.e9oe25();
    double gammaE=sqrt(px*px+py*py+pz*pz);

    if(gammaE<cuts::minGammaEBarrel) continue;  //  gamma energy cut
    HepLorentzVector boostedVec(px,py,pz,gammaE);
    Hep3Vector photVec(px,py,pz);

    // different e cuts for different theta region
    ...
    if(sector_gamma==2 && gammaE<0.100) continue;
    if(sector_gamma==4 && gammaE<0.050) continue;
    if(sector_gamma==6 && gammaE<0.075) continue;
    ...
    visEnergy_lab+=gammaE;//add the photon energy to Evis
    gammaCount++;
}
       // calculate thrust
    ...
    // Apply thrust angle cut in lab frame
    ...
    // Store thrust information in kinematics namespace for access throughout the analysis
   ...
   if(t.thru>m_thrustMagCut) return;// we do a test
    ...
    // Randomly flip thrust axis direction to avoid detector acceptance biases
    int randomFlag=-1;
    if(rand() % 100 <50) 
    { 
    ...
    float visEnergyOnFile=hadronInfo_mgr.begin()->Evis(); // two evis ?
    kinematics::E_miss=kinematics::Q-visEnergyOnFile;  // E_miss ?
    // define the E miss ?
  ...
  // if(cnt_pion<1) return;
    ...
    int NLambda=0;
     ...
     /*if goodLambda() = 1 : best S/sqrt(S+N) 
       ...
       without PID cut         
     */
    // Calculate angles between thrust axis and other reference axes
   ...
   for(Mdst_vee2_Manager::iterator vee_it=Mdst_vee2_Mgr.begin();vee_it!=Mdst_vee2_Mgr.end();vee_it++)
{
    ...
    Hep3Vector ch0 ((*vee_it).chgd(0).px(),(*vee_it).chgd(0).py(),(*vee_it).chgd(0).pz());  // .chgd()  , extract charged daughter
    Hep3Vector ch1 ((*vee_it).chgd(1).px(),(*vee_it).chgd(1).py(),(*vee_it).chgd(1).pz());
    
    // Create particle objects from the V0 candidates
    Particle L(*vee_it);
    Particle &child0 = L.relation().child(0);
    Particle &child1 = L.relation().child(1);
    Particle Pproton;
    ...
    if(m_PID&&m_prob_proton<0.6) continue; // PID cut on proton, m_PID ?
    ...
    // Calculate momentum fraction z and projection along thrust axis
    ...
    // Calculate angle between Lambda and thrust direction
    ...
    // Find hadrons in the opposite hemisphere for correlation studies
...
/**
 * Calculate the impact parameters and refitted momentum for a charged track.
 * @param chr_it Iterator pointing to the charged track
 * @param masshyp Mass hypothesis index (0=e, 1=μ, 2=π, 3=K, 4=p)
 * @param dr Reference to store the transverse impact parameter
 * @param dz Reference to store the longitudinal impact parameter
 * @param refitPx Reference to store the refitted x-momentum
 * @param refitPy Reference to store the refitted y-momentum
 * @param refitPz Reference to store the refitted z-momentum
 */
void LambdaAna::getDrDz(Mdst_charged_Manager::iterator chr_it, int masshyp, double& dr, double& dz, double& refitPx, double& refitPy, double& refitPz) {
    ...
    // Extract helix parameters from the track fit
    HepVector a(5, 0);
    ...
    // Extract the error matrix from the track fit
    HepSymMatrix Ea(5, 0);
 ...
 //sector selection for photons from Ami
  /**
   * Determine the ECL sector number based on the polar angle theta.
   * Used for photon selection to apply energy cuts based on detector region.
   * @param theta The polar angle in radians
   * @return Sector number (1-7), where:
   *         1: Forward endcap (inner)
   *         2: Forward endcap (outer)
   *         3: Gap between forward endcap and barrel
   *         4: Barrel
   *         5: Gap between barrel and backward endcap
   *         6: Backward endcap (outer)
   *         7: Backward endcap (inner)
   */
...
/**
 * Read Monte Carlo truth information and calculate MC-based event properties.
 * Identifies quark pairs, Lambda baryons, and calculates MC thrust axis.
 * Also performs event weighting for polarization studies.
 * 
 * @param m_qqaxis Reference to store the quark-antiquark axis
 * @param numquark Reference to store the number of quarks found
 * @return The Monte Carlo thrust axis
 */
...
/**
 * Calculate angles between the proton and reference directions in Lambda decay.
 * This method handles the relativistic transformations between different reference frames
 * and calculates the angular distributions needed for polarization analysis.
 * 
 * @param thrustDirCMS Thrust axis in center-of-mass frame
 * @param thrustDirLab Thrust axis in laboratory frame
 * @param Lambda_p4_Lab Lambda 4-momentum in laboratory frame
 * @param Proton_p4_Lab Proton 4-momentum in laboratory frame
 * @param Pion_p4_Lab Pion 4-momentum in laboratory frame
 * @param lptheta Reference to store the angle between proton and Lambda direction
 * @param costheta_test Reference to store test angle cosine
 * @param theta_fake Reference to store fake angle value
 * @param cost_fake Reference to store cosine of fake angle
 * @param cost_fake_b Reference to store cosine of alternate fake angle
 * @param cost_fake_c Reference to store cosine of third fake angle
 * @param cost_d Reference to store cosine of angle relative to beam direction
 * @param cost_fake_d Reference to store cosine of fake angle relative to beam
 * @param refdir_record Reference to store the reference direction
 * @return The decay angle theta_p
 */
double LambdaAna::CalTheta(
    Hep3Vector thrustDirCMS, 
    Hep3Vector thrustDirLab,  
    HepLorentzVector Lambda_p4_Lab, 
    HepLorentzVector Proton_p4_Lab, 
    HepLorentzVector Pion_p4_Lab, 
    double &lptheta, 
 ...
 * Calculate angles in the reference frame defined by a hadron.
 * This method transforms the proton direction into a frame where the Lambda-hadron plane is used 
 * as a reference, useful for correlation studies.
 * 
 * @param refHadron_p4_CMS 4-momentum of the reference hadron in CMS frame
 * @param Lambda_p4_Lab Lambda 4-momentum in laboratory frame
 * @param Proton_p4_Lab Proton 4-momentum in laboratory frame
 * @param pt_href Reference to store transverse momentum relative to hadron direction
 * @return Cosine of the angle between proton and the reference direction
 */
...
void LambdaAna::GetNhadron(int LheimFlag,Vp4 vp4kaon,Vp4 vp4pion, Vp4 vp4kaon_lab, Vp4 vp4pion_lab, Vint kaonchr, Vint pionchr, int LprotonTrkID, int LpionTrkID, Vint pionTrkID,Vint kaonTrkID, Vfloat& kpz, Vfloat& kmz, Vfloat& pipz, Vfloat& pimz, Vfloat& kp_oat, Vfloat& km_oat, Vfloat& pip_oat, Vfloat& pim_oat, Vint &kp_nMatch, Vint &km_nMatch, Vint &pip_nMatch, Vint &pim_nMatch, Vint &kp_trackIndex, Vint &km_trackIndex, Vint &pip_trackIndex, Vint &pim_trackIndex, Vfloat &vkp_mom_diff, Vfloat &vkm_mom_diff,Vfloat &vpip_mom_diff,Vfloat &vpim_mom_diff,Vfloat &vkp_angle_diff, Vfloat &vkm_angle_diff, Vfloat &vpip_angle_diff, Vfloat &vpim_angle_diff, Vp4 &vkp_oh_p4, Vp4 &vkm_oh_p4,Vp4 &vpip_oh_p4,Vp4 &vpim_oh_p4, Vp4 &vkp_oh_p4_matched, Vp4 &vkm_oh_p4_matched,Vp4 &vpip_oh_p4_matched,Vp4 &vpim_oh_p4_matched, Vint& kp_matchedType, Vint& km_matchedType, Vint& pip_matchedType, Vint& pim_matchedType, Vint& Vkaonindex, Vint& Vpionindex)
{
    ...
    Vkaonindex.clear();
    Vpionindex.clear();
    if(LheimFlag!=1&&LheimFlag!=-1) { 
        cout<<"somthing is wrong hemi of the Lambda is not decided "<<endl; 
    }
    if(LpionTrkID<0) { 
        cout<<"somthing is wrong TrkID of pion "<< LpionTrkID<<endl; 
        return;
    }
    if(LprotonTrkID<0) { 
        cout<<"somthing is wrong TrkID of proton "<< LprotonTrkID<<endl; 
        return;
    }
    ...
    for(int kk=0;kk<vp4kaon.size();kk++) {
        ...
        float opangle;
        if(LheimFlag==-1) 
            opangle = vp4kaon[kk].vect().angle(kinematics::thrustDirCM);//using the reversed thrust axis
        else 
            opangle = vp4kaon[kk].vect().angle(-1*kinematics::thrustDirCM);
        ...
        if(hrustProj*LheimFlag<0) {
            ...
            int htype=-1;
            if(m_mc==1)
                TrackMatch(vp4kaon_lab[kk], pkaon[kk], nMatch, trackIndex,mom_diff, angle_diff, p4_cms_match, htype);
            if(kaonchr[kk]>0) {  
                kpz.push_back(kaonz); 
                kp_oat.push_back(opangle); 
                kp_nMatch.push_back(nMatch); 
                kp_trackIndex.push_back(trackIndex); 
                vkp_mom_diff.push_back(mom_diff); 
                vkp_angle_diff.push_back(angle_diff); 
                vkp_oh_p4.push_back(vp4kaon[kk]); 
                vkp_oh_p4_matched.push_back(p4_cms_match); 
                kp_matchedType.push_back(htype);
            } else { 
                kmz.push_back(kaonz); 
                km_oat.push_back(opangle); 
                km_nMatch.push_back(nMatch); 
                km_trackIndex.push_back(trackIndex); 
                vkm_mom_diff.push_back(mom_diff); 
                vkm_angle_diff.push_back(angle_diff);
                vkm_oh_p4.push_back(vp4kaon[kk]);
                vkm_oh_p4_matched.push_back(p4_cms_match); 
                km_matchedType.push_back(htype);
            }
        }
    }
    ///pions
    ...
    for(int pp=0;pp<vp4pion.size();pp++) {
        ...
        float opangle;
        if(LheimFlag==-1) 
            opangle = vp4pion[pp].vect().angle(kinematics::thrustDirCM);//using the reversed thrust axis
        else 
            opangle = vp4pion[pp].vect().angle(-1*kinematics::thrustDirCM);
        ...
        if(hrustProj*LheimFlag<0) {
            ...
            int htype=-1;
            if(m_mc==1)
                TrackMatch(vp4pion_lab[pp], ppion[pp], nMatch, trackIndex,mom_diff,angle_diff, p4_cms_match, htype);
            if(pionchr[pp]>0) { 
                pipz.push_back(pionz); 
                pip_oat.push_back(opangle); 
                pip_nMatch.push_back(nMatch); 
                pip_trackIndex.push_back(trackIndex); 
                vpip_mom_diff.push_back(mom_diff); 
                vpip_angle_diff.push_back(angle_diff); 
                vpip_oh_p4.push_back(vp4pion[pp]); 
                vpip_oh_p4_matched.push_back(p4_cms_match); 
                pip_matchedType.push_back(htype);
            } else { 
                pimz.push_back(pionz); 
                pim_oat.push_back(opangle); 
                pim_nMatch.push_back(nMatch); 
                pim_trackIndex.push_back(trackIndex); 
                vpim_mom_diff.push_back(mom_diff); 
                vpim_angle_diff.push_back(angle_diff);
                vpim_oh_p4.push_back(vp4pion[pp]);
                vpim_oh_p4_matched.push_back(p4_cms_match); 
                pim_matchedType.push_back(htype);
...
int LambdaAna::MatchToMCTruth_new(int pcharge, int& mother, int &mothermother, int &isthep, HepLorentzVector Lp4_rec_lab,  Particle proton, Particle pion, Particle Lambda, double &cosl_true, double &cost_true, double &cost_true_qq, double &cost_d_true, int &Lindex, int &Prindex, int &Piindex, double &mom_diff, double &angle_diff, HepLorentzVector& Lp4_lab_true, HepLorentzVector& pp4_lab_true,HepLorentzVector& pip4_lab_true, Hep3Vector &refdir_true, int& numMatchquark, int& MatchFalvor)
{
    ...
    if(h_proton && h_pion && h_proton.mother() && h_pion.mother() && h_proton.mother().get_ID()==h_pion.mother().get_ID() &&abs(h_pion.mother().idhep())==3122) { 
        ...
        if(Itmother.mother()!=NULL) 
            mothermother = Itmother.mother().idhep();
        else 
            mothermother = -1;
        ...
        for(Gen_hepevt_Manager::iterator gen_it=gen_hep_Mgr.begin();gen_it!=gen_hep_Mgr.end();gen_it++) {
            bool isquark = false;
            if(abs(gen_it->idhep())==1||abs(gen_it->idhep())==2||abs(gen_it->idhep())==3||abs(gen_it->idhep())==4) { 
                isquark = true; 
            }
            if(isquark) {
        ...
        int lambdaHemei = 0;
        if(Lp4_CM.vect().angle(kinematics::thrustDirCMS_MC)<0.5*pi) 
            lambdaHemei = 1;
        else 
            lambdaHemei = -1;
        numMatchquark = 0;
        ...
        for(int hh = 0; hh<VquarkFlavor.size(); hh++) {
            int quarkhemi = 0;
            if(VquarkVecCM[hh].angle(kinematics::thrustDirCMS_MC)<0.5*pi) 
                quarkhemi = 1;
            else 
                quarkhemi = -1;
...
int LambdaAna::MatchToMCTruth(int pcharge, int& mother, int &mothermother, int& isthep, HepLorentzVector pp4, HepLorentzVector pip4, HepLorentzVector &pp4Matched_lab, HepLorentzVector &pip4Matched_lab, HepLorentzVector& Lp4Matched_lab, int &trkindex, double &mom_diff, double &angle_diff)
{
    ...
    for(Gen_hepevt_Manager::iterator gen_it=gen_hep_Mgr.begin(); gen_it!=gen_hep_Mgr.end(); gen_it++, index++) {
        ...
        if(ndaug!=2) continue;
        if(cnt_nproton!=1 || cnt_npion!=1) { 
            if(m_debug) cout<<" cannot pass number of pront, pion "<<cnt_nproton<<" ,  "<<cnt_npion<<endl; 
            continue;
        }
        if(pcharge_MC*pcharge<0) { 
            if(m_debug) cout<<" cannot pass charge  of proton "<<pcharge_MC<<" , "<<pcharge<<endl; 
            continue; 
        }

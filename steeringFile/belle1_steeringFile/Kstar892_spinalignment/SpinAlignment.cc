...
/*
-----------------------------------------
qqbar(udsc) -> K^*(892) / bar{K}^*(892) + anything                             
                \-> K+ pi- / K- pi+
...
void SpinAlignment::hist_def(){
    ...
    if (isMC){
        ...
        mctree->Branch("pim_E_cms_truth", &pim_E_cms_truth);  // pi- from K*0
        ...
        mctree->Branch("pip_E_cms_truth", &pip_E_cms_truth);  // pi+ from anti-K*0
    ...
    if(isMC){
        ...
        tree->Branch("n_kstar_truth", &n_kstar_truth);
...
for(Evtcls_hadron_neutral_Manager::iterator it = hadron_neu_mgr.begin();
    ...
    for(Mdst_charged_Manager::iterator ch_it=charged_mgr.begin(); ch_it!=charged_mgr.end(); ch_it++){
        ...
        if (isMC == 1 && (massHyp == 3 || massHyp == 2)) {
            ...
            bool fromKstar = (hasMother && abs(mc_mother_pdg) == 313);

            if (massHyp == 3) {  // kaon candidates
                bool isKaon = (mc_pdg * charge == 321);
                if (charge == 1) {
                    if (isKaon && fromKstar) {
            ...
            } else if (massHyp == 2) {  // pion candidates
                bool isPion = (mc_pdg * charge == 211);
                if (charge == 1) {  // pi+ (from anti-K*0)
                    if (isPion && fromKstar) {
    ...
    // ensure there has at least a K+pi- or K-pi+ pair
    if ((kp_px_cms.size() * pim_px_cms.size() + km_px_cms.size() * pip_px_cms.size()) == 0) return;  
...
void SpinAlignment::readMC()
{
    ...
    for(Gen_hepevt_Manager::iterator gen_it = gen_hep_Mgr.begin(); gen_it != gen_hep_Mgr.end(); ++gen_it) {
        ...
        if (abs(pid) == 313)
        {
            ...
            //if (ndaug != 2) continue; // K*(892) -> K pi only
            if (ndaug != 2){
                cout << "Warning: K*(892) meson decay daughters number is " << ndaug << endl;
            }
            ...
            for (int i = 0; i < ndaug; ++i){
                ...
                if (abs(daughter_pid) != 321 && abs(daughter_pid) != 211) continue; // only K and pi
                ...
                if (daughter_pid == 321){         // K+ (from K*0)
                ...
                else if (daughter_pid == -321){   // K- (from anti-K*0)
                ...
                else if (daughter_pid == -211){   // pi- (from K*0)
                ...
                else if (daughter_pid == 211){    // pi+ (from anti-K*0)
    ...
    n_kstar_truth = kp_E_cms_truth.size() + km_E_cms_truth.size();
    if (n_kstar_truth < 1) return; // ensure there is at least a K*(892) in truth level
...
// version log 
// v1.0.0 , first implementation by wangz , 2026-06-10
// change from phi spinalignment v2.1.4 
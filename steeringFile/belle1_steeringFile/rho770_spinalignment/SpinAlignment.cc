...
/*
-----------------------------------------
qqbar(udsc) -> rho(770) + anything                             
                \-> pi+ pi-                               

version : v1.0.0
Date    : 2026.06.10
Author  : Zhen Wang
// change from phi spin alignment v2.1.4
*/
...
void SpinAlignment::hist_def(){
    ...
    if(isMC){
        ...
        tree->Branch("n_rho_truth", &n_rho_truth);
...
for(Evtcls_hadron_neutral_Manager::iterator it = hadron_neu_mgr.begin();
    ...
    for(Mdst_charged_Manager::iterator ch_it=charged_mgr.begin(); ch_it!=charged_mgr.end(); ch_it++){
        ...
        if (isMC == 1 && massHyp == 2) {
            ...
            if (hasMother && abs(mc_mother_pdg) == 113)
                fromrho = true;

            if (mc_pdg * charge == 211)
                isPion = true;
...
void SpinAlignment::readMC()
{
    ...
    for(Gen_hepevt_Manager::iterator gen_it = gen_hep_Mgr.begin(); gen_it != gen_hep_Mgr.end(); ++gen_it) {
        ...
        if (pid == 113)
        {
            ...
            //if (ndaug != 2) continue; // rho -> pi+ pi- only
            if (ndaug != 2){
                cout << "Warning: rho meson decay daughters number is " << ndaug << endl;
            }
    ...
    n_rho_truth = pip_E_cms_truth.size();
    if (n_rho_truth < 1) return; // ensure there is at least a rho meson in truth level
...
// version log 
// v1.0.0 initial version, 2026-06-10
// change from phi spin alignment v2.1.4